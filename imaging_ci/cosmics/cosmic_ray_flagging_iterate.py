...
for i in range(iterations):
    ...
    cosmic_ray_mask = ac.Mask2D(
        mask=cosmic_ray_mask, pixel_scales=imaging_ci.pixel_scales
    )

    image_corrected = image_corrected.apply_mask(mask=cosmic_ray_mask)

figsize = (50, 40)
norm = colors.Normalize(vmin=0.0, vmax=norm)

output_path = path.join("imaging_ci", "cosmics", "output", f"{dataset_name}_iterate")
